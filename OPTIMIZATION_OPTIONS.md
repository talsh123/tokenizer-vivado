...
- **Correctness #2** ✅ **(verified on silicon 2026-06-22)** — the 1-char-word-after-multipiece merge
  bug fixed (`word_done_pending` single bit → `word_done_count` saturating counter); corpus
  **64/66 → 66/66** in sim and on hardware. The build was unblocked by disabling auto-incremental
  synthesis and loading the TEMAC Hardware-Eval license; the board returns 1037/1056 standing alone.
  Detail: `CODE_REVIEW.md` §8/§9, `JOURNAL.md` "#2 fix VERIFIED ON SILICON".
