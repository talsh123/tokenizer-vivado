...
**Build gotcha (module-ref staleness):** the tokenizer is a Vivado `module_ref`, and a Generate
Bitstream after editing its RTL can reuse the **cached pre-edit netlist** — the bitstream then keeps
old logic even though source + simulation are fixed (caught on-board 2026-06-21: the #2 fix was
sim-verified 66/66 but the first re-flashed board still showed the old merge). Fix: **Reset Output
Products → Generate Output Products → `reset_run synth_1`** before Generate Bitstream, and verify the
implemented design contains `word_done_count` (not `word_done_pending`) before flashing.
