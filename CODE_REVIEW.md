...
originally **97% (64/66)**; now **100% (66/66)** after the one-character-word fix below — **verified on
silicon 2026-06-22** (board returns the fixed `summarize a long`/`vocab t vocab` token streams). The
remaining differences are by-design, not silent:
...
- **One-character-word merge bug — ✅ FIXED (was 2/66 lines).** A one-character word that *immediately
  ...
  and the full corpus 64/66 → **66/66**, `inspect_mismatch.py` 0 mismatches; **verified on silicon
  2026-06-22**. RTL-only fix (no vocab change); detail in `JOURNAL.md` "Bug #2 fixed" / "#2 fix
  VERIFIED ON SILICON" and `analysis/evidence/MISMATCH_REPORT.md`.
...
|---|------|------------|-------|
| #2 | 1-char-word-after-multipiece merge | ✅ **Fixed in RTL, VERIFIED ON SILICON 66/66** (2026-06-22, `word_done_count` counter) — see §8. The build was blocked first by auto-incremental synthesis reusing the pre-fix partition (fixed by disabling incremental), then by the Tri-Mode Ethernet MAC bitstream license (fixed by loading the Hardware-Eval license); both resolved. Implemented netlist: `word_done_count` cells = 6, `word_done_pending` = 0. Board returns 1037/1056 standing alone. | `trie_engine.v` |
...
**Build gotcha (the real cause was AUTO-INCREMENTAL SYNTHESIS) — RESOLVED 2026-06-22:** after editing
tokenizer RTL, a rebuild can keep old logic in the bitstream even though source + simulation are fixed.
Caught on-board 2026-06-21/22: the #2 fix was sim-verified 66/66 but the re-flashed board still showed
the old merge. Root cause = **auto-incremental synthesis** (`AutoIncrementalCheckpoint="true"` in the
`.xpr`) reusing a pre-fix tokenizer partition from a stale reference checkpoint; Reset Output Products
did NOT fix it. After disabling incremental + a full rebuild (then loading the TEMAC Hardware-Eval
license so `write_bitstream` would run), the fix reached silicon and reverified 66/66.
